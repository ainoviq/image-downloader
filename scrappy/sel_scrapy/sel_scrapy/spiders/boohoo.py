...
from item_links.boohoo import boohoo_tees
...
class BoohooSpider(scrapy.Spider):
    ...
    def start_requests(self):
        print('No Filter = ', len(boohoo_tees))
        links = list(dict.fromkeys(boohoo_tees))
        ...
        for url in links:
            ...
            try:
                ...
                name = self.driver.find_elements_by_xpath(
                    '//h1[@class="b-product_details-name"]')[0].text
                img_btns = self.driver.find_elements_by_xpath(
                    '//div[@class="b-product_gallery-item "]')
                img_btns[0].click()
                time.sleep(5)

                imgs = self.driver.find_elements_by_xpath(
                    '//div[@class="b-photoswipe-thumbnails js-thumbnails"]/img')

                imgs_src = [img.get_attribute('src') for img in imgs]

                yield scrapy.Request(url=url, headers=self.headers, callback=self.parse_Boohoo, meta={'url': url, 'name': name, 'imgs_src': imgs_src}, dont_filter=True)
            except:
                print(f'ERROR: Failed to get response from url {url}')

    def parse_Boohoo(self, response):
        item = Item()
