...
from item_links.pretty_little_thing import plt_crop_tops, plt_basic_tops, plt_blouses, plt_cami_tops, plt_long_sleeve_tops, plt_shirts, plt_tshirts, plt_tank_tops


class PrettyLittleThingSpider(scrapy.Spider):
    name = 'plt'

    def __init__(self, category=None, url=None, *args, **kwargs):
        super(PrettyLittleThingSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        links = list(dict.fromkeys(plt_tank_tops))
        ...
        for url in links:
            ...
            try:
                ...
                name = self.driver.find_elements_by_xpath(
                    '//h1[@class="product-view-title"]')[0].text
                imgs = self.driver.find_elements_by_xpath(
                    '//img[@class="gallery-image js-click-galleryimage"]')
                time.sleep(1)

                imgs_src = [img.get_attribute('data-lazy') for img in imgs]
                imgs_src = list(dict.fromkeys(imgs_src))

                yield scrapy.Request(url=url, headers=self.headers, callback=self.parse_PLT, meta={'url': url, 'name': name, 'imgs_src': imgs_src}, dont_filter=True)
            except:
