...
from item_links.hoolister import polos


class HoolisterSpider(scrapy.Spider):
    name = 'hoolister'

    def __init__(self, category=None, url=None, *args, **kwargs):
        super(HoolisterSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        links = list(dict.fromkeys(polos))
        ...
        for url in links:
            ...
            try:
                ...
                name = self.driver.find_elements_by_xpath(
                    '//h1[@class="product-title-component product-title-main-header"]')[0].text
                
                imgs = self.driver.find_elements_by_xpath('//img[@class="thumbnail-image"]')
                imgs_src = [img.get_attribute('src').replace('product-small', 'product-extra-large') for img in imgs]
                imgs_src = list(dict.fromkeys(imgs_src))
                yield scrapy.Request(url=url, headers=self.headers, callback=self.parse_hoolister, meta={'url': url, 'name': name, 'imgs_src': imgs_src}, dont_filter=True)
            except:
                print(f'ERROR: Failed to get response from url {url}')

    def parse_hoolister(self, response):
        item = Item()
